...
const getProducts = async (req, res, next) => {
    try {
        ...
        if (req.query.price) {
            queryCondition = true;
            priceQueryCondition = { price: { $lte: Number(req.query.price) } }; //MongoDB less than equal $lte
        }

        let ratingQueryCondition = {};
        if (req.query.rating) {
            queryCondition = true;
            ratingQueryCondition = { rating: { $in: req.query.rating.split(",") } };//MongoDB $in include
        }
        ...
        if (req.query.category) {
            ...
            let a = req.query.category.split(",").map((item) => {
                if (item) return new RegExp("^" + item) //MongoDB ^ begin. of string
            })
        ...
        if (req.query.attrs) {
            ...
            attrsQueryCondition = req.query.attrs.split(",").reduce((acc, item) => {
                if (item) {
                    let a = item.split("-")
                    let values = [...a]
                    values.shift()   //removes first item in an arr
                    let a1 = {
...
const adminCreateProduct = async (req, res, next) => {    // Postman error beacuse post body raw json not selected
    try {
...
const adminUpload = async (req, res, next) => {   //error filesupload not working
