...
const getUsers = async (req, res, next) => {
    try {
        ...
        return res.json(users) // to do
    } catch (err) {
        next(err)
    }
    res.send("Handling  Users routes e.g. search for Users")
};
...
const registerUser = async (req, res, next) => {
    try {
        ...
        if (userExists) {
            ...
            res.cookie("access_token", generateAuthToken(user.id, user.name, user.lastName, user.email, user.isAdmin), {
                httpOnly: true,
...
//             return res.status(400).send("All inputs are required.")
//         }
//         //Create review id manually because it is needed also saving product in collection
//         const ObjectId = require("mongodb").ObjectId
//         let reviewId = ObjectId()
//         await Review.create([
//             {
...
//         const product = await Product.findById(req.params.prodcutId).populate("reviews");
